...
class AudioProcessorGUI:
    ...
    # ---------- Безопасные вызовы UI из любых потоков ----------

    def _in_main(self) -> bool:
        return threading.current_thread() is threading.main_thread()

    def ui(self, fn, *args, **kwargs):
        """Безопасный вызов fn(*args, **kwargs) в главном UI-потоке."""
        if self._in_main():
            fn(*args, **kwargs)
        else:
    ...
    def open_current_directory(self):
        path = str(self.current_work_dir)
    ...
    def refresh_files(self):
        def _do():
            ...
            audio_exts = {'.m4a', '.wav', '.mp3', '.ogg', '.opus'}
            text_exts = {'.txt'}
            ...
            for file_path, _ in files_with_time:
                ...
                size_str = f"{size // 1024 // 1024} МБ" if size > 1024 * 1024 else f"{max(1, size // 1024)} КБ"
    ...
    def run_command(self, cmd, cwd=None):
        """Запуск shell-команды в фоне с логированием вывода (UI безопасно)."""
        if cwd is None:
            cwd = self.base_dir
        ...
        try:
            ...
            for line in process.stdout:
                ...
                if not line:
                    continue
                # Просто логируем; без хитрых "перерисовок" строк — безопасно для UI
                self.log(line)
    ...
    def run_transcription(self, audio_file: Path):
        self.ui(self.progress_bar.config, mode='indeterminate')
        self.ui(self.progress_bar.start, 10)

        def on_progress(processed, total, elapsed, remaining):
            msg_parts = [f"Обработано: {processed}"]
            if total and total > 0:
                msg_parts.append(f"из {total}")
                # обновляем прогресс-бар только если известен total
                self.ui(self.progress_bar.stop)
                self.ui(self.progress_bar.config, mode='determinate', maximum=100,
                        value=(processed / total * 100))
            msg_parts.append(f"прошло: {elapsed:.1f}с")
            if remaining is not None and remaining > 0:
                msg_parts.append(f"осталось: {remaining:.1f}с")
    ...
    def transcribe_audio(self):
        ...
        if audio_file.suffix.lower() not in {'.m4a', '.wav', '.mp3', '.ogg', '.opus'}:
    ...
    def full_cycle(self):
        ...
        def worker():
            ...
            self.ui(self.progress_var.set, "Извлечение аудио...")
            cmd = f'ffmpeg -y -i "{input_file}" -vn -acodec copy "{audio_file}" -hide_banner'
...
# ---------- Whisper + tqdm с корректным прогрессом ----------
...
def transcribe_file(audio_path: Path, model_name: str = "large-v3",
    ...
    try:
        ...
        # Импортируем именно модуль whisper.transcribe, а не функцию
        wtr = importlib.import_module("whisper.transcribe")

        start_time = time.time()
        OldTqdm = wtr.tqdm  # tqdm-класс/ф-ция, которой пользуется whisper внутри

        class TqdmLogger(OldTqdm):
            def __init__(self, *args, **kwargs):
                self._cb = kwargs.pop("progress_callback", None)
                self._stop = kwargs.pop("stop_event", None)
            ...
            def _emit(self):
                ...
                remaining = (total - self.n) / rate if total and rate > 0 else None
                self._cb(self.n, total, elapsed, remaining)
        ...
        # Патчим tqdm прямо в модуле whisper.transcribe
        wtr.tqdm = lambda *a, **kw: TqdmLogger(*a, progress_callback=progress_callback,
                                               stop_event=stop_event, **kw)
        try:
            model = whisper.load_model(model_name)
            result = model.transcribe(str(audio_path))
        finally:
            # Возвращаем оригинальный tqdm
            wtr.tqdm = OldTqdm
